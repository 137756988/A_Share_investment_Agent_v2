...
from src.utils.llm_clients import SiliconFlowClient, LLMClientFactory
...
try:
    ...
    logger.warning(f"{WAIT_ICON} 未找到google.genai模块，将使用硅基流动API替代")
    use_gemini = False
...
def get_chat_completion(messages, model=None, max_retries=3, initial_retry_delay=1,
    ...
    Args:
        ...
        api_key: API 密钥（可选，用于各类API）
        base_url: API 基础 URL（可选，用于API请求）
        **kwargs: 其他参数，传递给底层API调用

    Returns:
        str: 模型回答内容或 None（如果出错）
    """
    try:
        # 如果没有gemini模块或者明确指定使用siliconflow
        if not use_gemini or client_type == "siliconflow" or client_type == "auto" and os.getenv("SILICONFLOW_API_KEY"):
            # 直接使用SiliconFlowClient
            logger.info(f"{WAIT_ICON} 使用硅基流动API")
